...
class SettingsConfiguration: NSObject, URLSessionDelegate, URLSessionTaskDelegate, URLSessionDataDelegate {
    ...
    private func setupMoppConfiguration(sivaUrl: String, tslUrl: String, tslCerts: [Data], tsaUrl: String, ocspIssuers: [String: String], certBundle: [Data]) -> Void {
        ...
        MoppLibConfiguration.sivaCert = CertUtil.certificate(CertUtil.getCertFile(folder: "siva-cert", fileName: DefaultsHelper.sivaCertFileName ?? ""))
        MoppLibConfiguration.tslURL = tslUrl
        MoppLibConfiguration.tslCerts = tslCerts
        MoppLibConfiguration.tsaURL = tsaUrl
        MoppLibConfiguration.tsaCert = CertUtil.certificate(CertUtil.getCertFile(folder: "tsa-cert", fileName: DefaultsHelper.tsaCertFileName ?? ""))
        MoppLibConfiguration.ocspIssuers = ocspIssuers
    ...
    private func reloadDigiDocConf() {
        MoppLibContainerActions.setup() { error in
            if error == nil {
    ...
    func saveLdapCerts(ldapCerts: [String], overwrite: Bool) {
        ...
        guard !ldapCerts.isEmpty else { printLog("No LDAP certs found (central configuration)"); return }
