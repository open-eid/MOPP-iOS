...
class IdCardViewController : MoppViewController {
    ...
    var idCardPersonalData: MoppLibPersonalData?
    var cert: Data?
    var pinStatus: (retryCount: UInt8, pinActive: Bool)?
    ...
    func updateUI(for state: State) {
        ...
        case .readyForTokenAction:
            ...
            DispatchQueue.main.asyncAfter(deadline: .now() + 2.5) {
                ...
                let (retryCount, pinActive) = self.pinStatus ?? (3, true)
                guard self.isActionDecryption || pinActive else {
                    return self.dismiss(animated: true) {
                        ErrorUtil.generateError(signingError: L(.pin2LockedAlert))
                    }
                }
                ...
                let pinHidden: Bool
                if !self.isActionDecryption, let cert = self.cert,
                ...
                self.pinTextField.isHidden = pinHidden
...
extension IdCardViewController : MoppLibCardReaderManagerDelegate {
    func moppLibCardReaderStatusDidChange(_ readerStatus: MoppLibCardReaderStatus) {
        ...
        case .CardConnected(let cardCommands):
            ...
            let isActionDecryption = self.isActionDecryption
            Task.detached { [weak self] in
                do {
                    let moppLibPersonalData = try await cardCommands.readPublicData()
                    let pinStatus = try await cardCommands.readCodeCounterRecord(isActionDecryption ? .pin1 :  .pin2)
                    let cert = try await (isActionDecryption ? cardCommands.readAuthenticationCertificate() : cardCommands.readSignatureCertificate())
                    guard let self else { return }
                    await MainActor.run {
