...
struct DigiDocConf final: public digidoc::ConfCurrent {
    ...
    std::vector<digidoc::X509Cert> toX509Certs(NSArray<NSData*> *certBundle, NSData *cert = nil) const {
...
+ (void)setupWith:(void (^_Nonnull)(NSError * _Nullable container))result {
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_HIGH, 0), ^{
        ...
        try {
            ...
            parseException(e);
            result([NSError errorWithDomain:@"MoppLib" code:e.code() userInfo:@{@"message":[NSString stringWithUTF8String:e.msg().c_str()]}]);
        }
    });
}
