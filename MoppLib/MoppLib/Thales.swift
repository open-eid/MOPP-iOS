...
class Thales: CardCommands {
    ...
    func readCodeCounterRecord(_ type: CodeType) throws -> NSNumber {
        ...
        if let info = TLV(from: Data(data)), info.tag == 0xA0 {
            for record in TLV.sequenceOfRecords(from: info.value) ?? [] where record.tag == 0xdf21 {
                return NSNumber(value: record.value[0])
            }
        }
        return 0
    }
    ...
    func unblockCode(_ type: CodeType, puk: String, newCode: String) throws {
        ...
        let pukCode = pinTemplate(puk)
        let unblockCmd = [0x00, 0x2C, 0x00, type.pinRef, UInt8(pukCode.count + newPin.count)]
        try errorForPinActionResponse(cmd: unblockCmd + pukCode + newPin)
    }
